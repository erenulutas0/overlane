...
AHighwayEnvironmentDirector::AHighwayEnvironmentDirector()
{
    ...
    // Optional scans. Absent, the pointers stay null and the flat-tint path runs.
    static ConstructorHelpers::FObjectFinder<UMaterialInterface> VergeScanFinder(
        TEXT("/Game/Environment/M_OverlaneVergeScan.M_OverlaneVergeScan"));
    static ConstructorHelpers::FObjectFinder<UMaterialInterface> GrassScanFinder(
        TEXT("/Game/Environment/M_OverlaneGrassScan.M_OverlaneGrassScan"));
    if (VergeScanFinder.Succeeded()) { VergeScanMaterial = VergeScanFinder.Object; }
    if (GrassScanFinder.Succeeded()) { GrassScanMaterial = GrassScanFinder.Object; }
...
void AHighwayEnvironmentDirector::BuildVisualRoute()
{
    ...
    // The scans already carry their own colour and tint, so they are used directly
    // rather than through CreateSurfaceMaterial's flat-tint path.
    UMaterialInterface* TerrainMaterial = GrassScanMaterial
        ? static_cast<UMaterialInterface*>(GrassScanMaterial)
        : CreateSurfaceMaterial(TerrainSurfaceMaterial, FLinearColor(0.085f, 0.115f, 0.06f));
    ...
    UMaterialInterface* VergeMaterial = VergeScanMaterial
        ? static_cast<UMaterialInterface*>(VergeScanMaterial)
        : CreateSurfaceMaterial(TerrainSurfaceMaterial, FLinearColor(0.105f, 0.092f, 0.070f));
