...
| P5-005 | in verification | Promote the practice bot into a standalone rival | P5-004 | Complete in code: forward sensing with the shared traffic following curve, real braking, overtaking via steered lane changes, stall-trap and spin recovery, damped speed-scaled steering, KOLAY/NORMAL/ZOR difficulty with turbo use and rubber banding, racer contact feedback, crimson body, and a HUD rival gap. Traffic now sees the bot for spawn safety, spawn window and lane-change safety. Compiles clean; **no runtime validation yet** - needs a solo race to confirm it holds a gap instead of grinding bumpers, overtakes cleanly, and does not weave at 180 km/h. |
