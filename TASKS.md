...
| P5-002 | pending | Add minimal session host/join flow | P5-001 | A host can create a local listen session and another client can join without manually entering a network address. |
...
| P5-005 | in progress | Promote the practice bot into a standalone rival | P5-004 | The server-only bot follows an outer lane in standalone solo play, can finish before the human player, stops the race, and reports a clear player-or-bot result. It remains excluded from multiplayer and does not receive score, boost, or lane-change behavior. |
